/* fiquei no https://viewsourcecode.org/snaptoken/kilo/07.syntaxHighlighting.html#detect-filetype */
...
void enableRawMode() {
	...
	raw.c_iflag &= ~(INPCK | BRKINT| ISTRIP | ICRNL | IXON);
	raw.c_oflag &= ~(OPOST);
	raw.c_cflag |= ~(CS8);
...
void editorFindCallback(char *query, int key) {
	...
	if (saved_hl) {
		mempcpy(E.row[saved_hl_line].hl, saved_hl, E.row[saved_hl_line].rsize);
		free(saved_hl);
		saved_hl = NULL;
	}
	...
	for (i = 0; i < E.numrows; i++) {
		...
		if (match) {
			...
			mempcpy(saved_hl, row->hl, row->rsize);
